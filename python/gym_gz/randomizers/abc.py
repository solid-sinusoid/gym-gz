...
class TaskRandomizer(abc.ABC):
    @abc.abstractmethod
    def randomize_task(self, task: gym_gz.base.task.Task, **kwargs) -> None:
        """
        Randomize a :py:class:`~gym_gz.base.task.Task` instance.

        Args:
            task: the task to randomize.

        Note:
            Note that each task has a :py:attr:`~gym_gz.base.task.Task.world`
...
class PhysicsRandomizer(abc.ABC):
    ...
    def randomize_physics(self, task: gym_gz.base.task.Task, **kwargs) -> None:
        ...
        :py:class:`~gym_gz.runtimes.gazebo_runtime.GazeboRuntime`.
